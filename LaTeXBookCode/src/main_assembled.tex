...
The evaluation field's own frameworks point the same way, which is why this chapter belongs in an evaluation book and not just a programming one. By the utilization-focused standard of Chapter~\ref{ch:embedded} \parencite{patton2008utilization}, a pipeline the program's own data manager can rerun after the contract ends is use in its most durable form. In the research-practice partnerships of Chapter~\ref{ch:embedded}, the project folder both sides can open, read, and rerun is the smallest artifact that makes the partnership's evidence jointly owned rather than held hostage to one analyst's laptop. The replicable principle, at team scale, is nothing more exotic than this: the project explains itself to the next person.
...
lets two measures on different scales (a rate per 100{,}000 and a
...
Keeping that history is instrument stewardship. It is the measurement side of improvement science: a plan-do-study-act cycle can compare this quarter's number to last quarter's only if the instrument held still, and a question silently reworded between cycles turns an apparent improvement into an artifact \parencite{bryk2015improve}. It is also what keeps the utilization-focused reporting of Chapter~\ref{ch:embedded} honest across waves \parencite{patton2008utilization}: the user acting on a trend line deserves to know whether the trend reflects the program or the question.
...
This safeguard is directly actionable, and its beneficiaries are the small sites themselves. Stabilized rates protect a five-person program from a statistical accident, which is what makes a public dashboard fair enough to publish. Shrinkage, in other words, is fairness expressed as arithmetic.
...
The Blinder--Oaxaca decomposition\index{Blinder-Oaxaca decomposition} is the standard tool for exactly this, and it is old and well understood: \textcite{blinder1973wage} and \textcite{oaxaca1973male} introduced it for wage gaps in the early 1970s, and it has been the workhorse of gap analysis ever since. Follow the logic in one pass. \emph{The problem}: a mean outcome differs between group $A$ and group $B$, and you want to split the difference. \emph{What you can see}: for each group, the outcome, the characteristics, and (by regression) what each characteristic is worth in that group. \emph{The key fact}: if you knew what each group \emph{would} earn under one common set of returns, the leftover would be pure structure. \emph{The move}: fit a pooled regression to get that common return $\beta^\ast$, then write the gap as
\[
...
The audit serves the people the targeting model acts on, which is the most consequential form of the accessible principle: a targeting tool that is fair is one the evaluator can defend to the community it affects. Checking prediction for bias is the duty of care that using prediction at all incurs.
