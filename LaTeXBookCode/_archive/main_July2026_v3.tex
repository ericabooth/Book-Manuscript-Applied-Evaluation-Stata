...
\begin{verbatim}
import delimited ///
    "https://data.bls.gov/cew/data/api/2023/1/area/48453.csv", ///
...
\begin{vizcallout}[Two Socrata traps: the \$ collision and silent truncation]
First, Socrata's row filter is spelled \texttt{\$limit}, and Stata reads \texttt{\$} as the start of a global macro, so \texttt{"...?\$limit=5000"} silently becomes \texttt{"...?=5000"} and the download fails. Build the URL with \texttt{char(36)} to insert a literal dollar sign: \texttt{local u = "...?" + char(36) + "limit=5000\&stateabbr=TX"}, then \texttt{import delimited "`u'"}. Second, without a limit Socrata returns only the first 1{,}000 rows, so a state's worth of tracts arrives truncated but looks complete. Set the limit above your expected row count and confirm \texttt{\_N} after import. Both are the kind of error that survives all the way to a published table.
\end{vizcallout}
...
\subsection{Two types worth showing: geo and table}
Two of the fourteen types cover most of what a board actually asks for. The geographic choropleth answers \enquote{show me my counties on a map,} and the searchable table answers \enquote{let me find my own row.} We show the syntax display-only, because the live renderer needs assets this page cannot fetch, but the commands are exactly what the do-file writes. The county-wage panel from Section~\ref{sec:sparkwall} feeds a Texas choropleth in one call:
{\footnotesize
\begin{verbatim}
* display only: writes wages_geo.html
googlechart avg_wkly_wage, type(geo) ///
    region("US-TX") resolution(provinces) ///
    id(area_fips) label(county) ///
    saving("$out/wages_geo.html")
\end{verbatim}
}
The \texttt{region("US-TX")} and \texttt{resolution(provinces)} options draw counties rather than states, and \texttt{id(area\_fips)} tells the renderer which column holds the geographic key. The same dataset becomes a searchable, sortable table with a different \texttt{type()}:
{\footnotesize
\begin{verbatim}
* display only: writes wages_table.html
googlechart county avg_wkly_wage ///
    month3_emplvl, type(table) ///
    format(avg_wkly_wage "$#,##0") ///
    saving("$out/wages_table.html")
\end{verbatim}
}
A client opens \texttt{wages\_table.html}, types their county name into the search box, and sorts by wage without a spreadsheet, which is the accessible promise served directly: the audience finds their own number instead of hunting a printed table. Both files are written by the do-file, so both rebuild when the wage panel refreshes, and neither is a hand-made artifact that can drift.\marginnote{The \texttt{geo} type is the one most likely to blank behind a firewall, because it fetches map tiles and boundary data at render time. Before you promise a client an air-gapped map, open the HTML with your network off and confirm it still draws; if it does not, ship the static choropleth from Chapter~\ref{ch:graphs} instead.}
...
\medskip
\textbf{The build.} From the single \texttt{ch10\_smallmult.do} panel, ship three forms of the same numbers. For the brief, the static spark wall of Figure~\ref{fig:sparkwall}, which needs nothing to render and cannot break. For the portal, the \texttt{googlechart type(geo)} choropleth and \texttt{type(table)} search from Section~\ref{sec:sparkwall}, vendored so they do not blank behind the agency firewall. For the talk, an animated bubble of wage against employment over the twenty-four quarters, used only because the trajectory is the point. Each deliverable is written by the same do-file from the same panel, so all three rebuild when next quarter's QCEW file posts. That is replicable (one do-file, three outputs), extensible (add a county, all three grow), accessible (each audience gets the form it can use), and actionable (the board plans in the counties and dollars every version reports).
\end{appliedexample}

\paragraph{Where this leaves you.} You can now build sparkline portfolios and spark walls, interactive charts, and infographics straight from a do-file, and you know when interactivity earns its keep versus when a static small-multiple already told the story. Because they come from code, these graphics stay replicable and easy to refresh, and because you build the static floor first, a blocked CDN never leaves you with nothing to show. Chapter~\ref{ch:sheets} turns to the format most clients actually live in, the spreadsheet, and how to deliver through it without giving up the pipeline.
...
In practice the same three lines you would run against a local workbook now target a Sheet the team already has open. You push the current summary, drop a live chart beside it, and read the sheet back to confirm the write landed where you meant:
{\footnotesize
\begin{verbatim}
googlesheets export summary using "SHEET_ID", ///
    sheet("Monday update") replace
googlesheets addchart using "SHEET_ID", ///
    sheet("Monday update") type(column) ///
    range("A1:C7")
googlesheets import using "SHEET_ID", ///
    sheet("Monday update") clear firstrow
\end{verbatim}
}
The re-import is not ceremony. It is the check that closes the loop: an evaluator who writes to a shared sheet and never reads it back is trusting that the API, the tab name, and the range all behaved, and any one of those can be quietly wrong.\marginnote{The nastiest silent failure is a partial write: the API returns success after landing the first few hundred rows, then the connection drops. The tab looks written, the row count is wrong, and only a read-back \texttt{count} catches it.} Reading the sheet back into Stata and comparing it against what you exported catches the silent write failure before the team sees a stale or half-written tab.
...
\textbf{Scenario.} A client wants one offline folder to share with their steering committee: a narrative report, interactive tables of site outcomes, and the impact graphs, and they need to know six months from now exactly which cut the committee approved.

\medskip
\textbf{The portal build.} Scaffold the workspace with \texttt{projectbuilder} (Chapter~\ref{ch:workshop}); run the analysis and export figures with \texttt{statplot} and \texttt{sparkta2} (Chapters~\ref{ch:graphs}, \ref{ch:interactive}); generate interactive tables and charts with \texttt{statashiny} into \texttt{charts/}; compile the narrative into \texttt{report.html} with \texttt{webdoc2}; and write an \texttt{index.html} landing page that links the pieces and carries the version-and-date stamp of Section~\ref{sec:versioning}. Tag the commit that built the folder and name the folder to match. The result is a folder the committee opens on any laptop, with no server and no logins, that the evaluator rebuilds next quarter in one command and that carries its own provenance so no copy is ever mistaken for another. The live example sites for these tools show the finished form.
\end{appliedexample}
