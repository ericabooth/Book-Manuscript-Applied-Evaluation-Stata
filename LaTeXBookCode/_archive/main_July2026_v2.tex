...
\paragraph{The four promises.} One idea runs through the book. Good evaluation work is \emph{replicable} (a colleague reruns it next month from the raw files and gets the same answer), \emph{extensible} (the next wave, site, or client fits without a rewrite), \emph{accessible} (partners and funders can open and understand the output), and \emph{actionable} (it tells someone what to do next Monday, in the units they budget in). These four words recur on purpose. Every section says which promises its methods serve, and why the reader's partners are better off for it.

\paragraph{Everything here runs.} This is not a book of pseudocode. Every worked example is a do-file in the companion \texttt{code/} folder, and every one downloads only public data, most of it with a single \texttt{import delimited} from a URL and no login. The figures in these pages were produced by those do-files. You can rerun them, change a county or a year, and watch the result change.
...
\begin{verbatim}
clear all
version 18                  // pin the language version for reproducibility
set more off
set varabbrev off           // require full variable names; abbreviations hide bugs

* THE ONE PLACE YOU EDIT: point this at your project folder.
global root "REPLACE/WITH/YOUR/PATH"

* Everything else is derived from root; you never touch these.
...
{\footnotesize
\begin{verbatim}
local run_all 0                    // flip to 1 to rebuild the whole project
if `run_all' {
...
The payoff is that the entire project rebuilds from raw download to final figure with one switch, which is the operational definition of replicable. When a reviewer asks where a number came from, the honest and fast answer is \enquote{run the pipeline and watch it appear,} not \enquote{let me find the right spreadsheet.}
...
Reproducibility is only bearable if a rerun is quick, so speed is a reproducibility feature, not a luxury. On files with millions of rows, the community packages \texttt{gtools} and \texttt{ftools} turn \texttt{collapse} into \texttt{gcollapse} and \texttt{egen} into \texttt{gegen} for large speed-ups, and \texttt{reghdfe} absorbs high-dimensional fixed effects that would otherwise stall. The bigger lever is filtering on import: the STAAR pipeline (Appendix~\ref{app:walkthroughs}) reads fourteen files of roughly 400\,MB each but keeps only the rows it needs, so the full file never sits in memory.

For teams on standard Stata, disciplined memory habits do most of the work: drop unneeded variables and rows right after ingestion, run \texttt{compress} before saving, and benchmark before optimizing. A pipeline that reruns in minutes gets rerun; one that takes a day gets patched by hand, and hand-patching is where reproducibility dies.

\paragraph{Where this leaves you.} Your projects now have a folder layout, a control file that holds every path in one place and rebuilds the pipeline on command, and habits that keep reruns fast and honest. Those are the replicable and extensible promises, banked once for every project ahead. Part~II turns to filling the project with data, beginning with the public APIs in Chapter~\ref{ch:apis}.
...
{\footnotesize
\begin{verbatim}
graph hbar (asis) avg_wkly_wage, over(county, sort(1) descending) ///
    blabel(bar, format(%9.0fc)) bar(1, color(navy)) ///
    ytitle("Average weekly wage, private sector ($), 2023 Q1")
graph export "$figures/ch03_qcew_wages.png", replace
\end{verbatim}
}
...
{\footnotesize
\begin{verbatim}
copy "https://www.countyhealthrankings.org/sites/default/files/media/document/analytic_data2024.csv" ///
...
The CDC's PLACES project is the other standing source, modeling roughly forty health measures down to the tract through Socrata endpoints that emit CSV directly. Socrata is worth knowing, and it carries two traps worth a boxed warning, one of them specific to Stata.
