...
Before you model a dataset, you size it up. Which variables are in it? How complete is each one? What do its codes mean, and what values does it usually take? Native Stata answers those questions one command at a time, \texttt{describe} for types, \texttt{summarize} for statistics, \texttt{codebook} for a fuller read, \texttt{tabulate} for categories, and you keep the picture in your head or a scratch file. \texttt{datadictionary}\index{datadictionary@\texttt{datadictionary}} builds the whole picture in one worksheet. Run it on the data in memory and it writes one row per variable that sets the statistics, percent missing, distinct count, common values, labels, and stored notes side by side, prints them to the screen, and exports them to an Excel workbook a colleague can open:
{\footnotesize
\begin{verbatim}
sysuse auto, clear
...
Read one screen and you know the file. The percent-missing column flags \texttt{rep78} at 6.8\,percent while every other variable is complete; the common-values column shows that \texttt{foreign} splits into Domestic and Foreign and that \texttt{make} takes 74 distinct string values; the statistics sit in the same row as the label that explains them. A varlist and an \texttt{if} narrow the codebook the usual way, \texttt{datadictionary price mpg rep78 if foreign == 1}, and each run ends by printing a clickable link that opens the workbook and its folder, so the file you just built is one click away.

That much works on any cross-sectional file. A survey that runs for years asks more: not just what this file contains, but what changed since last time. Return to the staff-wellbeing survey, fielded in 2019, 2021, and 2023 (180, 210, and 195 responses). Point \texttt{datadictionary} at the three wave files and it stacks a row for every variable in every wave and adds a \texttt{changed} column that flags any variable whose label or categories shifted from the wave before:
{\footnotesize
\begin{verbatim}
...
Figure~\ref{fig:datadictionaryxlsx} shows the result, and reading down the columns tells the study's story. The percent-missing column tracks the self-reported \texttt{hours} item across waves, 5.0\,percent missing in 2019, 16.7 in the pandemic wave of 2021, back to 8.7 in 2023, the kind of wave-specific nonresponse to weigh before reading a trend as real. The common-values column shows \texttt{q2} holding four categories under \texttt{agree4} in 2019 and five under \texttt{agree5} in 2023, so the recode shows up in the data, not just the label name. And the \texttt{changed} column earns its place: it flags \texttt{q1} in 2023, whose label was reworded, and \texttt{q2} in 2023, whose category set was replaced, each a warning that the variable may no longer mean what it meant in 2019. The \texttt{Changes} sheet lists every such difference in full, with the before and after side by side; here \texttt{r(nchanges)} returns 4.
...
Two details in that block carry the whole method. The do-file the command writes is not a black box; it is readable Stata that a reviewer can open, and every line is wrapped in \texttt{capture} with \texttt{varabbrev} turned off at the top, so a column the collaborator renamed or dropped is skipped rather than fuzzily matched to the wrong variable. And the export uses \texttt{nolabel}, which sends the numeric codes rather than the label text, so the \texttt{label values} lines in the do-file have codes to reattach to; export the words \enquote{Disagree} instead and the column comes back as a string that can no longer be re-encoded. Figure~\ref{fig:datadictionary-roundtrip} traces the full loop.
...
\begin{itemize}
    ...
    \item \textbf{The twelve packages built for this book}, each in its own \texttt{-stata-public} repository with tests and help: \texttt{projectbuilder} (scaffold the Chapter~\ref{ch:workshop} project layout), \texttt{combineall} (append, merge, or convert a whole folder, with vintage-aware harmonization; Chapters~\ref{ch:apis} and~\ref{ch:noapi}), \texttt{cxchangelog} (cross-wave survey codebooks), \texttt{datadictionary} (the over-time data codebook: per-wave stats, missingness, value-label diffs, Excel export), and \texttt{undummy} (recombine one-hot columns into one categorical), all Chapter~\ref{ch:surveys}; \texttt{rateshrink} (empirical-Bayes rate stabilization), \texttt{conformalpred} (distribution-free prediction intervals), and \texttt{hlmr2} (Nakagawa multilevel $R^2$), all Chapter~\ref{ch:trust}; \texttt{twinmatch} (Mahalanobis policy twins) and \texttt{roisim} (Monte Carlo ROI), both Chapter~\ref{ch:claims}; and \texttt{suppress} (small-cell and complementary suppression) and \texttt{riskscan} ($k$-anonymity scan), both Chapter~\ref{ch:sharing}.
